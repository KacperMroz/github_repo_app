...
void main() {
  ...
  group('GetPullRequestForRepoUseCase', () {
    final tParam =
        GetPullRequestForRepoParams(owner: 'test_owner', repo: 'test_repo');
    const tPullRequestsList =
        PullRequestsList(items: [PullRequest(id: 1, title: 'Pull Request 1')]);
    const tAppError = AppError.unknown();

    test(
        'should return PullRequestsList when the call to repository is successful',
        () async {
      ...
      verify(
        () => mockPullRequestRepository.getPullRequestsForRepo(
          tParam.owner,
          tParam.repo,
        ),
      );
